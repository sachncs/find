...
| `end_to_end_small_scalar_12345` (10M scalars) | ~5.27 ms | 1.88 ms | 2.80× |

All numbers above come from a single canonical `cargo bench` run
on an Apple M3 Pro (`opt-level=3`, `lto=fat`, `criterion` 10
...
The post-precomputed-tables numbers include the [match_x](src/search.rs)
optimisation from commit `000843a` (slice-typed `test_x` argument,
drops one 32-byte copy per probe inside the per-batch match loop).
...
| `VariantIndex::match_x` arg `test_x` | `&[u8; 32]` | `&[u8]` (ADR-0012; drops 32-byte copy per probe) |
