...
When writing or reviewing documentation, follow the published style
guides that this project has adopted:

- [Google Developer Documentation Style Guide](https://developers.google.com/style) — voice and tone, grammar, formatting, link text, headings (sentence case), lists, code samples, accessibility.
- [Archbee's overview of technical writing style guides](https://www.archbee.com/blog/technical-writing-style-guide) — broader reference for clarity, scannable structure, terminology tables, and inclusive language.

Key rules the project enforces on top of those guides:

- Use **sentence case** for headings (`### Batch normalization`, not `### Batch Normalization`).
- Use **active voice** and **second person** ("you can run...", not "we run...").
- Put conditions **before** instructions ("If you have Rust 1.81+, ..."), not after.
- Use **descriptive link text** — never "click here" or bare URLs.
- Use **em dashes** (`—`) with spaces around them for parenthetical emphasis.
- Use **serial commas** in lists of three or more items.
- Wrap **code-related text** in backticks; reserve **bold** for UI element names.
