...
- **`VariantIndex::match_x` takes `test_x: &[u8]`** (was `&[u8; 32]`)
  [ADR-0012](docs/adr/0012-match-x-slice-signature.md). Drops one
  32-byte `memcpy` per probe inside the per-batch match loop of
  `sweep_parallel` and `sweep_and_cache`. Measured −5 % on
  `end_to_end_small_scalar_12345` and `plus_g_chain/chain_32_plus_g`
  on Apple M3 Pro. Breaking change tracked for `0.2.0`.
