...
| [0011](0011-address-discovery.md) | Address-discovery input mode: `--address <base58>` + `--from X --to Y` switch to a hash40-targeted scalar sweep over the user range. Strict Base58Check; mainnet P2PKH/P2SH only; u64 scalar range. | Accepted |
| [0012](0012-match-x-slice-signature.md) | `VariantIndex::match_x` takes `test_x: &[u8]` (was `&[u8; 32]`); drops one 32-byte `memcpy` per probe inside the per-batch match loop of `sweep_parallel` / `sweep_and_cache`. ~5 % end-to-end speedup on Apple M3 Pro. | Accepted |
