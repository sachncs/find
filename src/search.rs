...
impl VariantIndex {
    ...
    /// # Arguments
    ///
    /// * `test_x` — A 32-byte big-endian X-coordinate to search for. May
    ///   be any byte slice (typically a `FieldBytes` from k256); the
    ///   search only inspects the first 32 bytes.
    ...
    ///
    /// `test_x` is taken by `&[u8]` (not `&[u8; 32]`) so callers can pass
    /// `affine.x().as_ref()` directly without copying into a stack
    /// array first — saves one 32-byte copy per probe in the hot loop.
    #[inline(always)]
    pub fn match_x(&self, test_x: &[u8], j: u128) -> Option<SearchMatch> {
        // Use a fixed-size key view for the binary search; falls back to
        // a per-byte compare if `test_x` is shorter than 32 bytes.
        let idx = self
            .keys
            .binary_search_by(|probe| probe.as_slice().cmp(test_x))
            .ok()?;
        ...
        while bi < sb_count {
            ...
            for gbi in 0..group_batch_count {
                ...
                for i in 0..count {
                    ...
                    if let Some(m) = index.match_x(affine.x().as_ref(), j) {
                ...
                for gbi in 0..group_batch_count {
                    ...
                    for i in 0..count {
                        ...
                        if let Some(idx_ref) = index {
                            if let Some(m) = idx_ref.match_x(x_bytes.as_ref(), j) {
                                local_match = Some(m);
                                break;
                            }
                        }

                        block[block_len..block_len + 32].copy_from_slice(x_bytes.as_ref());
                        block_len += 32;
                    }
